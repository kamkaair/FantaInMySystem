	...
	vec3 getNormalFromMap()
	{
		...
		vec3 B = normalize(cross(N, T)); // changed to match the forward's cross, was: vec3 B = -normalize(cross(N, T)); 
		mat3 TBN = mat3(T, B, N);
	...
	{    
		...
		// Both metallic and Roughness values into a singular sampler2D:
