	...
	void main()
	{
		vec3 fragPosu   = texture(gPosition, texCoords).xyz;
		...
		{
		    // get sample position
			vec3 samplePos = fragPosu + TBN * samples[i] * radius; // from tangent to view-space
			...
			// Range check
			float rangeCheck = smoothstep(0.0, 1.0, radius / abs(fragPosu.z - sampleDepth));
