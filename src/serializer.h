...
public: 
	// Vectors
	template<typename T>
	void write(std::ofstream& file, const std::vector<T>& vec) {
	...
	template<typename T>
	void read(std::ifstream& file, std::vector<T>& vec) {
		size_t size;
	...
	// Standard optional
	template<typename T>
	void write(std::ofstream& file, const std::optional<T> inString) {
	...
	template<typename T>
	void read(std::ifstream& file, std::optional<T>& stringCache) {
	...
	void write(std::ofstream& file, const std::string& inString) {
	...
	void read(std::ifstream& file, std::string& stringCache) {
		// Read 8 bytes from the file and copy into memory
		size_t nameLength;
		file.read(reinterpret_cast<char*>(&nameLength), sizeof(nameLength));
	...
	// Generic format
	template<typename T>
	void write(std::ofstream& file, const T& inValue) {
		static_assert(std::is_trivially_copyable_v<T>);
		file.write(reinterpret_cast<const char*>(&inValue), sizeof(T));
	}

	template<typename T>
	void read(std::ifstream& file, T& outValue) {
		static_assert(std::is_trivially_copyable_v<T>);
